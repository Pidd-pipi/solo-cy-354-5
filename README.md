...
覆盖：买家确认生成 6 位一次性码（仅买家可见、带有效期）、错误码不完成订单、过期码被拒、重复核销仅成功一次、**16 个并发核销只有一次成功**，以及商品状态更新失败时整笔核销事务回滚——回滚用例通过数据库触发器在 `UPDATE products SET status='sold'` 语句本身注入失败（商品行与订单关联均保留），断言订单不落为完成、面交码不被消费、商品保持在售，且解除故障后同单同码可正常核销成功。
