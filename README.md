...
面交核销测试使用纯 Go（无 CGO）的 SQLite + 真实 GORM 仓储/事务，**无需外部 MySQL**，在干净环境直接可跑、可重复复跑：

```bash
# 服务层（真实事务/回滚/并发）
go test ./internal/service/ -run TestHandover -v -count=1
# HTTP 接口层（真实 Gin 路由 + JWT 鉴权 + 中间件 + 统一错误码）
go test ./internal/handler/ -run TestAPI -v -count=1
```

- 服务层覆盖：买家确认生成 6 位一次性码（仅买家可见、带有效期）、错误码不完成订单、过期码被拒、重复核销仅成功一次、**16 个并发核销只有一次成功**，以及商品状态更新失败时整笔核销事务回滚——回滚用例通过数据库触发器在 `UPDATE products SET status='sold'` 语句本身注入失败（商品行与订单关联均保留），断言订单不落为完成、面交码不被消费、商品保持在售，且解除故障后同单同码可正常核销成功。
- 接口层（`internal/handler/trade_order_handover_api_test.go`，`httptest` + `router.New` 装配真实引擎）覆盖：买家确认后**仅买家响应包含面交码**（卖家/第三方响应为空）、卖家核销成功、错误码 40910、过期码 40911、重复核销 40912、待确认无码 40913、重新生成仅买家可用（卖家/路人/匿名 403/401）、参数校验 42200、未登录 40100、未知订单 40400，以及并发 HTTP 核销单赢家。
